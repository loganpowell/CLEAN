...
export const args = {
  name: ({ name }) => {
    ...
    const filter_array = (mult && name.map(n => ({ name: { contains: n } }))) || []
  ...
  code: ({ code }) => {
    ...
    const filter_array = (mult && code.map(n => ({ code: { eq: n } }))) || []
  ...
  id: ({ id }) => {
    ...
    const filter_array = (mult && id.map(n => ({ id: { contains: n } }))) || []
  ...
  dates: ({ bgn, end }) => {
    ...
    const date = {
      created_at:
        bgn && end ? { between: [bgn, end] } : bgn ? { gt: bgn } : end ? { eq: end } : null,
    }
  ...
  sender: ({ sender }) => {
    ...
    const filter_array = (mult && sender.map(n => ({ sender_email: { contains: n } }))) || []
  ...
  camp: ({ camp }) => {
    ...
    const filter_array = (mult && camp.map(n => ({ campaign_id: { contains: n } }))) || []
...
export const listSubs = ({
  topic: { name = undefined, code = undefined, id = undefined, limit: t_limit = 100 },
  bulletin: { bgn = undefined, end = undefined, limit: b_limit = 100, sort = "DESC" },
}) =>
  jq(
    subs({
  ...
  grafetch({
    ...
    ({
      ...
      items
        ...
        .sort((pre, cur) => new Date(cur.created_at).getTime() - new Date(pre.created_at).getTime())
  )
...
export const fetchBulletins = ({
  ...
  grafetch({
    ...
    ({
      ...
      squash(items).sort(
        (pre, cur) => new Date(cur.created_at).getTime() - new Date(pre.created_at).getTime()
      )
  )
